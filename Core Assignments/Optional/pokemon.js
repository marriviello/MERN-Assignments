...
const poisonType = pokémon.filter(p => p.types.includes("poison") && p.types.length === 1);
console.log(poisonType);

// an array containing just the first type of all the pokémon whose second type is "flying"
const flyingType = pokémon.filter(p => p.types.includes("flying")).map(p => p.types[0]);
console.log(flyingType)

// a count of the number of pokémon that are "normal" type
const norm = pokémon.filter(p=>p.types.includes("normal"));
